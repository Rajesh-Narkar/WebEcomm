...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <SortedDescendingHeaderStyle BackColor="#275353" />--%>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:dbconn %>" SelectCommand="SELECT * FROM [user_account] WHERE ([acc_role] = @acc_role)" ConflictDetection="CompareAllValues" DeleteCommand="DELETE FROM [user_account] WHERE [id] = @original_id AND (([acc_user] = @original_acc_user) OR ([acc_user] IS NULL AND @original_acc_user IS NULL)) AND (([acc_pass] = @original_acc_pass) OR ([acc_pass] IS NULL AND @original_acc_pass IS NULL)) AND (([acc_email] = @original_acc_email) OR ([acc_email] IS NULL AND @original_acc_email IS NULL)) AND (([acc_role] = @original_acc_role) OR ([acc_role] IS NULL AND @original_acc_role IS NULL))" InsertCommand="INSERT INTO [user_account] ([acc_user], [acc_pass], [acc_email], [acc_role]) VALUES (@acc_user, @acc_pass, @acc_email, @acc_role)" OldValuesParameterFormatString="original_{0}" ProviderName="<%$ ConnectionStrings:dbconn.ProviderName %>" UpdateCommand="UPDATE [user_account] SET [acc_user] = @acc_user, [acc_pass] = @acc_pass, [acc_email] = @acc_email, [acc_role] = @acc_role WHERE [id] = @original_id AND (([acc_user] = @original_acc_user) OR ([acc_user] IS NULL AND @original_acc_user IS NULL)) AND (([acc_pass] = @original_acc_pass) OR ([acc_pass] IS NULL AND @original_acc_pass IS NULL)) AND (([acc_email] = @original_acc_email) OR ([acc_email] IS NULL AND @original_acc_email IS NULL)) AND (([acc_role] = @original_acc_role) OR ([acc_role] IS NULL AND @original_acc_role IS NULL))">
            <DeleteParameters>
